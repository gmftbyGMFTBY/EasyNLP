...
def inference(**args):
    ...
    elif work_mode in ['unparallel']:
        # 1. use response encoder to build index
        # 2. use context encoder to encode the responses
        agent.inference(data_iter, size=args['cut_size'])
        agent.inference_context_for_response(data_iter, size=args['cut_size'])
