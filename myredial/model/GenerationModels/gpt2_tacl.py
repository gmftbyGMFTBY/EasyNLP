...
class GPT2TaCLV3Encoder(nn.Module):

    def __init__(self, **args):
        super(GPT2TaCLV3Encoder, self).__init__()
        model = args['pretrained_model']
        gpt2_model = args['gpt2_pretrained_model']
        self.vocab = BertTokenizer.from_pretrained(model)
        self.pad = self.vocab.pad_token_id
    ...
    def _encode(self, ids, ids_mask):
        gpt2_logits_1, gpt2_rep_1 = self.gpt2_encoder(ids, ids_mask)
    ...
    def forward(self, batch):
        ...
        # lm loss and token acc
        lm_loss, token_acc = self.get_lm_loss_and_token_acc(gpt2_logits_1, gpt2_ids)

        # token-aware contrastive training loss
        # cosine similarity
        # bert_rep: [B, S, E]; gpt2_rep: [B, S, E]; gpt2_ids_mask: [B, S]
        gpt2_rep_1 = F.normalize(gpt2_rep_1, dim=-1)
        gpt2_rep_2 = F.normalize(gpt2_rep_2, dim=-1)
        cosine_sim = torch.bmm(gpt2_rep_1, gpt2_rep_2.permute(0, 2, 1))    # [B, S, S]
        cosine_sim /= self.temp
