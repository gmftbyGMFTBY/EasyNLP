...
class BERTDualCompEncoder(nn.Module):
    ...
    def _encode(self, cid, rid, cid_mask, rid_mask):
        ...
        b_c = len(cid_rep)
        rid_reps = rid_rep.unsqueeze(1).repeat(1, b_c, 1).permute(1, 0, 2)    # [B_c, B_r*gray, E]
        ...
        rid_reps = gate_score * rid_reps + (1-gate_score) * comp_reps
        # rid_reps: [B_c, G, E]; cid_rep: [B_c, E]
        rid_reps = rid_reps.permute(1, 0, 2)    # [G, B_c, E]
    ...
    def forward(self, batch):
        ...
        assert b_c == b_r
        # cid_rep: [B_c, E]; rid_rep: [B_r*gray, E]
        cid_rep, rid_rep = self._encode(cid, rid, cid_mask, rid_mask)
        dot_product = torch.einsum('ijk,jk->ij', rid_rep, cid_rep).t()    # [B_c, G]
        dot_product /= np.sqrt(768)     # scale dot product
