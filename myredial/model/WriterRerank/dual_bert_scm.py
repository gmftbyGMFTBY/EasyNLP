...
class BERTDualSCMHNEncoder(nn.Module):
    ...
    def __init__(self, **args):
        ...
        self.can_encoder = BertEmbedding(model=model)
        # fusion layer
        if args['use_encoder_hidden_states']:
            decoder_layer = nn.TransformerDecoderLayer(d_model=768, nhead=args['nhead'])
            self.fusion_encoder = nn.TransformerDecoder(decoder_layer, num_layers=args['num_layers'])
        else:
        ...
        self.use_encoder_hidden_states = args['use_encoder_hidden_states']
...
class BERTDualSCMDistHNEncoder(nn.Module):
    ...
    '''distributed collect, cannot use the encoder hidden states in fusion encoder'''
    ...
    def _encode(self, cid, rid, cid_mask, rid_mask, is_test=False):
        rid_size = len(rid)
        cid_size = len(cid) if is_test else len(cid) * dist.get_world_size()
        # cid_rep_whole: [B_c*Card, S, E];
        cid_rep_whole = self.ctx_encoder(cid, cid_mask, hidden=True)
        if is_test is False:
            cid_rep_whole = distributed_collect_encoder_hidden_states(cid_rep_whole, max_size=self.args['max_len'])
            cid_mask = distributed_collect_mask(cid_mask, max_size=self.args['max_len'])
        # cid_rep: [B_c*Card, E]; cid_rep_: [B_c*Card, 1, E]
        cid_rep = cid_rep_whole[:, 0, :]
        cid_rep_ = cid_rep.unsqueeze(1)
        # rid_rep: [B_r*K, E]
        if is_test:
            rid_rep = self.can_encoder(rid, rid_mask)
        else:
            rid_rep = self.can_encoder(rid, rid_mask)
            # rid_rep_whole: [B_r*Card, K, E]
            rid_rep_whole = torch.stack(torch.split(rid_rep, self.topk))
        ...
        ## combine context and response embeddings before comparison
        if self.context_before_comparison:
            # rep_cid_backup: [B_r*Card, B_c*Card, E]
            rep_rid = rid_rep.unsqueeze(1).expand(-1, cid_size, -1)
            rep_cid = cid_rep.unsqueeze(0).expand(len(rep_rid), -1, -1)
            # rep: [B_r*Card, B_c*Card, E]
            rep = rep_cid + rep_rid
        else:
            # rid_rep: [B_r*Card, B_c*Card, E]
            rep = rid_rep.unsqueeze(1).expand(-1, cid_size, -1)
        # cid_rep_whole: [S, B_c*Card, E]
        cid_rep_whole = cid_rep_whole.permute(1, 0, 2)
        rest = self.fusion_encoder(
            rep,
            cid_rep_whole,
        ...
        # rest: [B_c*Card, E, B_r*Card]
        rest = rest.permute(1, 2, 0)
        # dp: [B_c*Card, B_r*Card]
        dp = torch.bmm(cid_rep_, rest).squeeze(1)
        if is_test:
            return dp
        ...
        ### hard negative comparison
        if self.context_before_comparison:
            # rep_rid: [K, B_r*Card, E]
            rep_rid = rid_rep_whole.permute(1, 0, 2)
            # rep_cid: [K, B_c*Card, E]
            rep_cid = cid_rep.unsqueeze(0).expand(len(rep_rid), -1, -1)
            # rep: [B_r*Card, B_c*Card, E]
            rep = rep_cid + rep_rid
        else:
        ...
        # rest: [K, B_r*Card, E] -> [B_r*Card, E, K]
        rest = rest.permute(1, 2, 0)
        # dp: [B_c*Card, K]
        dp2 = torch.bmm(cid_rep_, rest).squeeze(1)
        return dp, dp2
    ...
    def forward(self, batch):
        ...
        acc = (dp.max(dim=-1)[1] == torch.LongTensor(torch.arange(batch_size*dist.get_world_size())).cuda()).to(torch.float).mean().item()
        return loss, acc


class BERTDualSCMDMHNEncoder(nn.Module):

    '''dynamic mask'''

    def __init__(self, **args):
        super(BERTDualSCMDMHNEncoder, self).__init__()
        model = args['pretrained_model']
        self.ctx_encoder = BertEmbedding(model=model)
        self.can_encoder = BertEmbedding(model=model)
        # fusion layer
    ...
    def get_tgt_mask(self, batch_size):
        mask = torch.zeros(batch_size, batch_size)
        index = list(range(batch_size))
        for i in range(batch_size):
            num = min(batch_size, random.randint(self.args['min_allow_dm'], self.args['max_allow_dm']))
            select_index = random.sample(index, num)
            mask[i, select_index] = 1.
        mask[range(batch_size), range(batch_size)] = 1.
    ...
    def _encode(self, cid, rid, cid_mask, rid_mask, is_test=False):
        ...
        if self.use_encoder_hidden_states:
            if is_test:
                ...
                tgt_mask = self.get_tgt_mask(len(rep))
            rest = self.fusion_encoder(
                rep, 
                cid_rep_whole,
                tgt_mask=tgt_mask,
...
class BERTDualSCMSDLHNEncoder(nn.Module):

    def __init__(self, **args):
        super(BERTDualSCMSDLHNEncoder, self).__init__()
        model = args['pretrained_model']
        self.ctx_encoder = BertEmbedding(model=model)
        self.can_encoder = BertEmbedding(model=model)
        # fusion layer
    ...
    def _encode(self, cid, rid, cid_mask, rid_mask, is_test=False):
        ...
        # rep: [B_r, B_c, E]
        rep = rep_cid + rep_rid
        dp2_b = torch.bmm(cid_rep_, rep_rid.permute(1, 2, 0)).squeeze(1)    # [B_c, K]
        # rest: [K, B_r, E]
        rest = self.fusion_encoder(
            rep, 
            cid_rep_whole,
        ...
        # dp: [B_c, K]
        dp2 = torch.bmm(cid_rep_, rest).squeeze(1)
        return dp, dp2, dp2_b, cid_rep, rid_rep
    ...
    def forward(self, batch):
        ...
        # [B_c, B_r]
        loss = 0
        dp, dp2, dp2_b, cid_rep, rid_rep = self._encode(cid, rid, cid_mask, rid_mask)
        # before comparsion, optimize the absolute semantic space
        dot_product = torch.matmul(cid_rep, rid_rep.t())
        mask = torch.zeros_like(dot_product)
        mask[range(batch_size), range(batch_size)] = 1.
        ...
        # sharpen distribution loss
        dp_ad = F.softmax(dp, dim=-1)
        dp_bd = F.softmax(dot_product, dim=-1)
        sdl = dp_bd.diag() - dp_ad.diag()
        sdl = sdl[sdl > 0]
        loss += sdl.sum()

        dp2_ad = F.softmax(dp2, dim=-1)
        dp2_bd = F.softmax(dp2_b, dim=-1)
        sdl = dp_bd.diag() - dp_ad.diag()
        sdl = sdl[sdl > 0]
        loss += sdl.sum()
        # end of sdl
