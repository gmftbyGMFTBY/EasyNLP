...
def ranking(context_hidden, next_hidden, next_top_k_ids, scoring_criterion):
    '''
        context_hidden: beam_width x context_len x embed_dim
        next_hidden: beam_width x 1 x embed_dim
        next_top_k_ids: beam_width x 1
        scoring_cirterion: average or maximum; the string type
    '''
    beam_width, context_len, embed_dim = context_hidden.size()
    norm_context_hidden = context_hidden / context_hidden.norm(dim=2, keepdim=True)
    norm_next_hidden = next_hidden / next_hidden.norm(dim=2, keepdim=True)
    cosine_matrix = torch.matmul(norm_context_hidden, norm_next_hidden.transpose(1,2)).squeeze(-1)    # [B, S]
...
def ContrastiveDecodingOneStep(model, input_ids, beam_width, scoring_criterion):
    '''
        model: the generation model, e.g., gpt2
        input_ids: 1 x seqlen; in-place appendation
    '''
    output = model(input_ids=input_ids, output_hidden_states=True)
    prev_hidden_states = output.hidden_states[-1]    # [B, S, E]
    ...
    next_id = ranking(context_hidden, next_hidden, top_k_ids, scoring_criterion)
