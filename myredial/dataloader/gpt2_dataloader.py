...
class GPT2UnlikelyhoodDataset(Dataset):
    ...
    def collate(self, batch):
        if self.args['mode'] == 'train':
            ...
            neg_ids_mask = generate_mask(neg_ids)
            ids, mask, pos_ids, pos_ids_mask, neg_ids, neg_ids_mask, pos_label, neg_label = to_cuda(ids, mask, pos_ids, pos_ids_mask, neg_ids, neg_ids_mask, pos_label, neg_label)
            return {
                'ids': ids, 
                'mask': mask, 
                'pos_ids': pos_ids, 
