...
class BERTDualPTDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        if self.args['mode'] == 'train':
            ...
            for label, utterances in tqdm(data):
                ...
                num_valid_ctx = len([i for i in ids if i not in [self.sep, self.cls]])
                num_valid_res = len([i for i in rids if i not in [self.sep, self.cls]])
                if num_valid_ctx < self.args['min_len'] or num_valid_res < self.args['min_len']:
                    continue
                self.data.append({
                    'ids': ids,
                    'rids': rids,
    ...
    def __getitem__(self, i):
        ...
        if self.args['mode'] == 'train':
            ...
            mask_labels_ids = mask_sentence_only_mask(
                ids,
            ...
            mask_labels_rids = mask_sentence_only_mask(
                rids,
                self.args['min_mask_num'],
                self.args['max_mask_num'],
                self.args['masked_lm_prob'],
