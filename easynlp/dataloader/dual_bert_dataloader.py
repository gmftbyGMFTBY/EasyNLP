...
class PersonaBERTDualFullDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        self.pp_path = f'{os.path.splitext(path)[0]}_persona_dual_full_{suffix}.pt'
        ...
        if self.args['mode'] == 'train':
            data = read_text_data_utterances_full_persona(path, lang=self.args['lang'])
            ...
            for i in tqdm(range(0, len(data), 20)):
                batch = data[i:i+20]
                rids = []
                gt_text = []
...
class BERTDualO2MDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        self.data = []
        if self.args['mode'] == 'train':
            path = f'{os.path.splitext(path)[0]}_dpr_gray.txt'
            data = read_text_data_one2many(path, lang=self.args['lang'])
            for context, response, candidates in tqdm(data):
                ctx = self.vocab.batch_encode_plus(context, add_special_tokens=False)['input_ids']
                ids = []
                for u in ctx:
                    ids.extend(u + [self.sep])
                ids.pop()
                ...
                rids = self.vocab.batch_encode_plus([response] + candidates, add_special_tokens=False)['input_ids']
                rids = [[self.cls] + i[:(self.args['res_max_len']-2)] + [self.sep] for i in rids]
                self.data.append({
                    'context': ids,
                    'response': rids[0],
                    'candidates': rids[1:],
    ...
    def __getitem__(self, i):
        bundle = self.data[i]
        if self.args['mode'] == 'train':
            ids = torch.LongTensor(bundle['context'])
            rids = torch.LongTensor(bundle['response'])
            cids = [torch.LongTensor(i) for i in random.sample(bundle['candidates'], self.topk)]
            return ids, [rids] + cids
        else:
            ids = torch.LongTensor(bundle['ids'])
            rids = [torch.LongTensor(i) for i in bundle['rids']]
    ...
    def collate(self, batch):
        if self.args['mode'] == 'train':
            ...
            for i in range(self.topk+1):
                ids_ = [item[1][i] for item in batch]
...
class BERTDualBertMaskHardNegativeDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        if self.args['mode'] == 'train':
            ...
            data = read_text_data_utterances(path, lang=self.args['lang'])
            if self.args['dataset'] in ['ubuntu'] and self.args['mode'] == 'valid':
                # too many samples, random sampled some during training
                data = data[:10000]
            for i in tqdm(range(0, len(data), 10)):
                batch = data[i:i+10]
                rids = []
    ...
    def collate(self, batch):
        if self.args['mode'] == 'train':
            ...
            for i in batch:
                crids.extend(i[1])
            ids = pad_sequence(ids, batch_first=True, padding_value=self.pad)
            rids = pad_sequence(crids, batch_first=True, padding_value=self.pad)
            ids_mask = generate_mask(ids)
            rids_mask = generate_mask(rids)
            ids, rids, ids_mask, rids_mask = to_cuda(ids, rids, ids_mask, rids_mask)
            return {
...
class BERTDualFullMultiContextDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        suffix = args['tokenizer'].replace('/', '_')
        self.pp_path = f'{os.path.splitext(path)[0]}_dual_full_multi_ctx_{suffix}.pt'
        if os.path.exists(self.pp_path):
            self.data = torch.load(self.pp_path)
            print(f'[!] load preprocessed file from {self.pp_path}')
            return None
        ...
        if self.args['mode'] == 'train':
            ...
            for label, utterances in tqdm(data):
                ...
                item = self.vocab.batch_encode_plus(utterances, add_special_tokens=False)['input_ids']
                cids, rids = item[:-1], item[-1]
                cids = self.cut_cids(cids)
                ids = []
                for idx in range(len(cids)):
                    if len(cids[idx:]) > args['max_turn_length_k']:
                        continue
                    ids_ = []
                    for u in cids[idx:]:
                        ids_.extend(u + [self.sep])
                    ids_.pop()
                    ids_ = ids_[-(self.args['max_len']-2):]    # ignore [CLS] and [SEP]
                    ids_ = [self.cls] + ids_ + [self.sep]
                    ids.append(ids_)
                rids = rids[:(self.args['res_max_len']-2)]
                rids = [self.cls] + rids + [self.sep]
                self.data.append({
    ...
    def cut_cids(self, cids):
        tokens_num = 1    # [CLS]
        max_turn_length = 0
        for u in cids[::-1]:
            tokens_num += len(u) + 1   # [SEP]
            max_turn_length += 1
            if tokens_num > self.args['max_len']:
                break
        cids = cids[-max_turn_length:]
        return cids
    ...
    def collate(self, batch):
        if self.args['mode'] == 'train':
            ...
            # batch size is batch_size * 10
            assert len(batch) == 1
            ids, rids, label, ids_length = batch[0]
            ids = pad_sequence(ids, batch_first=True, padding_value=self.pad)
            rids = pad_sequence(rids, batch_first=True, padding_value=self.pad)
            rids_mask = generate_mask(rids)
            ids_mask = generate_mask(ids)
...
class BERTDualBM25SCMDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        suffix = args['tokenizer'].replace('/', '_')
        self.pp_path = f'{os.path.splitext(path)[0]}_dual_bm25_scm_{suffix}.pt'
        if os.path.exists(self.pp_path):
            self.data = torch.load(self.pp_path)
            print(f'[!] load preprocessed file from {self.pp_path}')
            return None
    ...
    def __getitem__(self, i):
        ...
        if self.args['mode'] == 'train':
            ids, rids = bundle['ids'], bundle['rids']
            if self.args['easy_gray']:
                hrids = [j['rids'] for j in random.sample(self.data, self.args['gray_cand_num'])]
            else:

                hrids = random.sample(bundle['crids'], self.args['gray_cand_num'])
            ids = torch.LongTensor(ids)
            rids = torch.LongTensor(rids)
            hrids = [torch.LongTensor(i) for i in hrids]
            return ids, [rids] + hrids
...
class BERTDualBM25SCMLiteDataset(Dataset):
...
class BERTDualFullCorruptSCMDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        assert self.args['deletion_num'] + self.args['concatention_num'] == self.args['gray_cand_num']

        suffix = args['tokenizer'].replace('/', '_')
        self.pp_path = f'{os.path.splitext(path)[0]}_dual_full_corrupt_scm_{self.args["full_turn_length"]}_{suffix}.pt'
        if os.path.exists(self.pp_path):
            self.data = torch.load(self.pp_path)
            print(f'[!] load preprocessed file from {self.pp_path}')
            return None
    ...
    def corrupt(self, ids):
        # random deletion 
        sample_idx = max(self.args['deletion_min_num'], int(self.args['deletion_ratio'] * len(ids)))
        mask_index = list(range(len(ids)))
        random.shuffle(mask_index)
        mask_index = mask_index[:sample_idx]
        new_ids_deletion = [t for idx, t in enumerate(ids) if idx not in mask_index]
        # random concatention
        new_ids_concatention = []
        for _ in range(self.args['concatention_num']):
            random_index = random.choice(range(len(self.data)))
            utterance = self.data[random_index]['rids']
            save_length = len(ids) // 2
            cat_length = len(ids) - save_length
            if random.random() < 0.5:
                # left and right
                ids_ = ids[:save_length] + utterance[-cat_length:]
            else:
                ids_ = utterance[:cat_length] + ids[-save_length:]
            new_ids_concatention.append(ids_)
        return [new_ids_deletion] + new_ids_concatention

    def __getitem__(self, i):
        bundle = self.data[i]
        if self.args['mode'] == 'train':
            ids = torch.LongTensor([self.cls] + bundle['ids'] + [self.sep])
            rids = torch.LongTensor([self.cls] + bundle['rids'] + [self.sep])
            hn_rids = [torch.LongTensor([self.cls] + j + [self.sep]) for j in self.corrupt(bundle['rids'])]
            return ids, [rids] + hn_rids, bundle['ctext'], bundle['rtext']
        else:
            ids = torch.LongTensor(bundle['ids'])
            rids = [torch.LongTensor(i) for i in bundle['rids']]
            return ids, rids, bundle['label'], bundle['text']
    ...
    def collate(self, batch):
        if self.args['mode'] == 'train':
            ...
            for a, b, c, d in batch:
                b = [torch.LongTensor(i) for i in b]
                rids.extend(b)
            ctext = [i[2] for i in batch]
            rtext = [i[3] for i in batch]
            ids = pad_sequence(ids, batch_first=True, padding_value=self.pad)
...
class BERTDualFullDailyDialogDataset(Dataset):
    ...
    def __init__(self, vocab, path, **args):
        ...
        self.data = []
        if self.args['mode'] == 'train':
            data = read_text_data_utterances_full_dailydialog(path, lang=self.args['lang'], turn_length=self.args['full_turn_length'])
            ...
            data = read_text_data_utterances_dailydialog(path, lang=self.args['lang'])
            for i in tqdm(range(0, len(data), 10)):
                batch = data[i:i+10]
                rids = []
                gt_text = []
...
class BERTRUBERDualDataset(Dataset):
