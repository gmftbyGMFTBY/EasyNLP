...
class LatentInteractionAgent(RetrievalBaseAgent):
    ...
    def __init__(self, vocab, model, args):
        ...
        if args['model'] in ['colbertv2tacl']:
    ...
    def train_model_tacl(self, train_iter, test_iter, recoder=None, idx_=0, whole_batch_num=0):
        self.model.train()
        total_cl_loss, total_tacl_loss, total_acc, batch_num = 0, 0, 0, 0
        pbar = tqdm(train_iter)
        correct, s = 0, 0
        for idx, batch in enumerate(pbar):
            self.optimizer.zero_grad()
            with autocast():
                cl_loss, tacl_loss, acc = self.model(batch)
                loss = cl_loss + tacl_loss 
            ...
            if recoder:
                ...
                recoder.add_scalar(f'train-epoch-{idx_}/TaCLLoss', total_tacl_loss/batch_num, idx)
                recoder.add_scalar(f'train-epoch-{idx_}/Acc', total_acc/batch_num, idx)

            pbar.set_description(f'[!] loss(cl|tacl): {round(total_cl_loss/batch_num, 4)}|{round(total_tacl_loss/batch_num, 4)}; acc: {round(total_acc/batch_num, 4)}')
        if recoder:
            recoder.add_scalar(f'train-whole/CLLoss', total_cl_loss/batch_num, idx_)
            recoder.add_scalar(f'train-whole/TaCLLoss', total_tacl_loss/batch_num, idx_)
