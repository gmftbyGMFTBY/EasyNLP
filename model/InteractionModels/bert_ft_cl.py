...
class BERTCLRetrieval(nn.Module):

    def __init__(self, model='bert-base-chinese', p=0.2):
        super(BERTCLRetrieval, self).__init__()
    ...
    def forward(self, inpt_1, token_type_ids_1, attn_mask_1, inpt_2, token_type_ids_2, attn_mask_2):
        def encode(inpt, attn_mask, token_type_ids):
            output = self.model(
                input_ids=inpt,
                attention_mask=attn_mask,
                token_type_ids=token_type_ids,
            )[0]    # [B, S, E]
            return self.proj(output[:, 0, :])
        inpt1_embd = encode(inpt_1, attn_mask_1, token_type_ids_1)    # [B, 768]
        inpt2_embd = encode(inpt_2, attn_mask_2, token_type_ids_2)    # [B, 768]
